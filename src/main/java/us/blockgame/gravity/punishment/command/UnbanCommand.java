...
public class UnbanCommand {
    ...
    @SneakyThrows
    @Command(name = "unban", permission = "gravity.command.unban", runAsync = true)
    public void unban(CommandArgs args) {
        ...
        JSONObject activeBan = punishmentHandler.getActiveBan(uuid);

        //Make sure player is banned
        if (activeBan == null) {
            args.getSender().sendMessage(ChatColor.RED + "That player is not banned.");
            return;
        }

        activeBan.put("active", false);
        ...
        //Replace punishment
        punishments.put(activeBan.get("time").toString(), activeBan);
        //Replace punishments
        ...
        //Broadcast unban
        FancyMessage fancyBanMessage = new FancyMessage(silent ? "(Silent) " : "").color(ChatColor.GRAY).then(senderRank.getLitePrefix() + args.getSender().getName()).then(" has unbanned ").color(ChatColor.GREEN).then(playerRank.getLitePrefix() + cacheHandler.getUsername(uuid)).then(".").color(ChatColor.GREEN);
